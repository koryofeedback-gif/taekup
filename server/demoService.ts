...
export async function clearDemoData(clubId: string): Promise<{ success: boolean; message: string; deletedCount: number }> {
  try {
    ...
    for (const table of relatedTables) {
      try {
        await db.execute(sql.raw(
          `DELETE FROM ${table} WHERE student_id IN (SELECT id FROM students WHERE club_id = '${clubId}' AND is_demo = true)`
        ));
      } catch (tableErr: any) {
        console.warn(`[DemoService] Could not clean ${table}:`, tableErr.message);
      }
    }
    ...
    // Step 3: Reset club — clear wizard_data (removes demo coaches) and reset wizard so user sees demo choice again
    await db.execute(sql`
      UPDATE clubs 
