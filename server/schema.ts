...
// =====================================================
// SCHEDULE - Classes, Private Slots, Events
// =====================================================

export const scheduleClasses = pgTable('schedule_classes', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  clubId: uuid('club_id').references(() => clubs.id, { onDelete: 'cascade' }).notNull(),
  coachId: uuid('coach_id').references(() => coaches.id, { onDelete: 'set null' }),
  ...
  color: varchar('color', { length: 50 }).default('sky'),
...
export const schedulePrivateSlots = pgTable('schedule_private_slots', {
  id: uuid('id').primaryKey().default(sql`gen_random_uuid()`),
  clubId: uuid('club_id').references(() => clubs.id, { onDelete: 'cascade' }).notNull(),
  coachId: uuid('coach_id').references(() => coaches.id, { onDelete: 'set null' }),
  ...
  bookedByStudentId: uuid('booked_by_student_id').references(() => students.id, { onDelete: 'set null' }),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow(),
});
...
export const scheduleEvents = pgTable('schedule_events', {
  ...
  clubId: uuid('club_id').references(() => clubs.id, { onDelete: 'cascade' }).notNull(),
  title: varchar('title', { length: 255 }).notNull(),
  eventType: varchar('event_type', { length: 50 }).default('social'),
  date: varchar('date', { length: 20 }).notNull(),
...
export type NewScheduleClass = typeof scheduleClasses.$inferInsert;
export type SchedulePrivateSlot = typeof schedulePrivateSlots.$inferSelect;
export type NewSchedulePrivateSlot = typeof schedulePrivateSlots.$inferInsert;
