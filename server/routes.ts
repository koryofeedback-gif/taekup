...
export function registerRoutes(app: Express) {
  ...
  app.get('/api/super-admin/broadcast/audience', async (req: Request, res: Response) => {
    try {
      ...
      return res.json({ count: rows.length, sample });
    } catch (err: any) {
  ...
  app.post('/api/super-admin/broadcast/send', async (req: Request, res: Response) => {
    try {
      ...
      const unsubscribeFooter = `<br><br><hr style="border:none;border-top:1px solid #374151;margin:24px 0"><p style="font-size:11px;color:#6b7280;text-align:center">You are receiving this email because you use MyTaek. <a href="https://app.mytaek.com/unsubscribe?email={{email}}" style="color:#9ca3af">Unsubscribe</a> | <a href="https://app.mytaek.com/privacy" style="color:#9ca3af">Privacy Policy</a><br>© ${new Date().getFullYear()} MyTaek Inc.</p>`;
      ...
      for (let i = 0; i < rows.length; i += BATCH_SIZE) {
        ...
        const personalizations = batch.map((r: any) => ({
        ...
        try {
          await sgMail.sendMultiple({
            ...
            subject,
            html: body + unsubscribeFooter,
            text: body.replace(/<[^>]*>/g, ''),
          } as any);
          sent += batch.length;
          batches++;
        } catch (batchErr: any) {
          console.error('[Broadcast] Batch error:', batchErr?.response?.body || batchErr.message);
          errors += batch.length;
        }
