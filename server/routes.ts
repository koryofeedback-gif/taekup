...
export function registerRoutes(app: Express) {
  ...
  // =====================================================
  // SCHEDULE API - Classes, Private Slots, Events
  // =====================================================

  // Validate UUID format (prevents invalid inputs reaching DB)
  const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  function isValidUUID(val: unknown): val is string {
    return typeof val === 'string' && UUID_RE.test(val);
  }

  // Verify caller is an active owner/coach of the given club.
  // Checks that userId belongs to an active user with club_id = clubId.
  async function verifyClubAccess(clubId: string, userId: string): Promise<boolean> {
    const r = await db.execute(sql`
      SELECT id FROM users
      WHERE id = ${userId}::uuid AND club_id = ${clubId}::uuid AND is_active = true
    `);
    return (r as any[]).length > 0;
  }

  // Ensure schedule tables exist (run once at startup via these endpoints)
  async function ensureScheduleTables() {
    ...
    `);
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS schedule_private_slots (
        id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        club_id UUID NOT NULL REFERENCES clubs(id) ON DELETE CASCADE,
        coach_id UUID REFERENCES coaches(id) ON DELETE SET NULL,
  ...
  ensureScheduleTables().catch(e => console.error('[Schedule] Table init error:', e.message));

  // --- CLASSES ---
  app.get('/api/schedule/classes', async (req: Request, res: Response) => {
    try {
      const { clubId, userId } = req.query;
      if (!isValidUUID(clubId) || !isValidUUID(userId)) return res.status(400).json({ error: 'Valid clubId and userId required' });
      if (!await verifyClubAccess(clubId, userId)) return res.status(403).json({ error: 'Access denied' });

      const classes = await db.execute(sql`
        SELECT sc.*, 
          COUNT(s.id)::int AS enrolled_count
        FROM schedule_classes sc
        LEFT JOIN students s ON s.club_id = sc.club_id 
          AND s.assigned_class = sc.class_name
          AND s.assigned_class IS NOT NULL
        WHERE sc.club_id = ${clubId}::uuid AND sc.is_active = true
        GROUP BY sc.id
      ...
      console.error('[Schedule] Get classes error:', error.message);
      return res.status(500).json({ error: 'Failed to get classes' });
    }
  });

  app.post('/api/schedule/classes', async (req: Request, res: Response) => {
    try {
      const { clubId, userId, coachId, className, coachName, category, color, dayOfWeek, startTime, endTime, maxCapacity, location, beltRequirement } = req.body;
      if (!isValidUUID(clubId) || !isValidUUID(userId) || !className || !dayOfWeek || !startTime) {
        return res.status(400).json({ error: 'Valid clubId, userId, className, dayOfWeek, startTime are required' });
      }
      if (!await verifyClubAccess(clubId, userId)) return res.status(403).json({ error: 'Access denied' });

      const coachIdValue = isValidUUID(coachId) ? coachId : null;
      const result = await db.execute(sql`
        INSERT INTO schedule_classes (club_id, coach_id, class_name, coach_name, category, color, day_of_week, start_time, end_time, max_capacity, location, belt_requirement)
        VALUES (
          ${clubId}::uuid,
          ${coachIdValue}::uuid,
          ${className},
      ...
      console.error('[Schedule] Create class error:', error.message);
      return res.status(500).json({ error: 'Failed to create class' });
    }
  });

  app.delete('/api/schedule/classes/:id', async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const { clubId, userId } = req.query;
      if (!isValidUUID(id) || !isValidUUID(clubId) || !isValidUUID(userId)) return res.status(400).json({ error: 'Valid id, clubId, and userId required' });
      if (!await verifyClubAccess(clubId, userId as string)) return res.status(403).json({ error: 'Access denied' });

      await db.execute(sql`
        DELETE FROM schedule_classes WHERE id = ${id}::uuid AND club_id = ${clubId}::uuid
      `);
      return res.json({ success: true });
    } catch (error: any) {
      console.error('[Schedule] Delete class error:', error.message);
      return res.status(500).json({ error: 'Failed to delete class' });
    }
  });

  // --- PRIVATE SLOTS ---
  app.get('/api/schedule/private-slots', async (req: Request, res: Response) => {
    try {
      const { clubId, userId } = req.query;
      if (!isValidUUID(clubId) || !isValidUUID(userId)) return res.status(400).json({ error: 'Valid clubId and userId required' });
      if (!await verifyClubAccess(clubId, userId)) return res.status(403).json({ error: 'Access denied' });
      ...
      const slots = await db.execute(sql`
        SELECT * FROM schedule_private_slots
        WHERE club_id = ${clubId}::uuid
        ORDER BY date, time
      `);
      return res.json(slots);
    } catch (error: any) {
      console.error('[Schedule] Get private slots error:', error.message);
      return res.status(500).json({ error: 'Failed to get private slots' });
    }
  });

  app.post('/api/schedule/private-slots', async (req: Request, res: Response) => {
    try {
      const { clubId, userId, coachId, coachName, date, time, price } = req.body;
      if (!isValidUUID(clubId) || !isValidUUID(userId) || !coachName || !date || !time) {
        return res.status(400).json({ error: 'Valid clubId, userId, coachName, date, time are required' });
      }
      if (!await verifyClubAccess(clubId, userId)) return res.status(403).json({ error: 'Access denied' });

      const slotCoachId = isValidUUID(coachId) ? coachId : null;
      const result = await db.execute(sql`
        INSERT INTO schedule_private_slots (club_id, coach_id, coach_name, date, time, price)
        VALUES (
          ${clubId}::uuid,
          ${slotCoachId}::uuid,
          ${coachName},
      ...
      console.error('[Schedule] Create private slot error:', error.message);
      return res.status(500).json({ error: 'Failed to create private slot' });
    }
  });

  app.delete('/api/schedule/private-slots/:id', async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const { clubId, userId } = req.query;
      if (!isValidUUID(id) || !isValidUUID(clubId) || !isValidUUID(userId)) return res.status(400).json({ error: 'Valid id, clubId, and userId required' });
      ...
      await db.execute(sql`
        DELETE FROM schedule_private_slots WHERE id = ${id}::uuid AND club_id = ${clubId}::uuid
      `);
      return res.json({ success: true });
    } catch (error: any) {
      console.error('[Schedule] Delete private slot error:', error.message);
      return res.status(500).json({ error: 'Failed to delete private slot' });
    }
  });
  ...
  app.get('/api/schedule/events', async (req: Request, res: Response) => {
    try {
      ...
      console.error('[Schedule] Get events error:', error.message);
  ...
  app.post('/api/schedule/events', async (req: Request, res: Response) => {
    try {
      const { clubId, userId, title, eventType, date, time, location, description } = req.body;
      if (!isValidUUID(clubId) || !isValidUUID(userId) || !title || !date) {
        return res.status(400).json({ error: 'Valid clubId, userId, title, date are required' });
      }
      if (!await verifyClubAccess(clubId, userId)) return res.status(403).json({ error: 'Access denied' });

      const result = await db.execute(sql`
        INSERT INTO schedule_events (club_id, title, event_type, date, time, location, description)
        VALUES (
          ${clubId}::uuid,
          ${title},
          ${eventType || 'social'},
      ...
      console.error('[Schedule] Create event error:', error.message);
  ...
  app.delete('/api/schedule/events/:id', async (req: Request, res: Response) => {
    try {
      ...
      await db.execute(sql`
        DELETE FROM schedule_events WHERE id = ${id}::uuid AND club_id = ${clubId}::uuid
      `);
      return res.json({ success: true });
    } catch (error: any) {
      console.error('[Schedule] Delete event error:', error.message);
