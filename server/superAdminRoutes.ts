...
// Master SendGrid template ID for all Super Admin emails
...
const EMAIL_TEMPLATES: Record<string, SuperAdminEmailTemplate> = {
  ...
  win_back: {
    ...
    getDynamicData: (club) => ({
      ...
      name: club.owner_name || 'there',
      clubName: club.name,
      body: `It's been a while since we've seen you at <strong>${club.name}</strong>. We'd love to have you back!<br><br>As a special offer, use code <strong>WINBACK25</strong> to get 25% off for your first 3 months. Your students are waiting to continue their martial arts journey!`,
      btn_text: 'Come Back & Save 25%',
  ...
  churn_risk: {
    ...
    getDynamicData: (club) => ({
      ...
      name: club.owner_name || 'there',
      clubName: club.name,
      body: `We noticed you might need some help getting started with TaekUp for <strong>${club.name}</strong>.<br><br>Our team is here to assist you every step of the way. Whether you need help adding students, setting up your schedule, or configuring your belt system - we've got you covered!`,
      btn_text: 'Get Help Now',
    ...
    try {
      ...
      if (sgClient) {
        ...
        const welcomeI18n: Record<string, { subject: string; title: string; greeting: string; congrats: string; trialNote: string; credTitle: string; emailLabel: string; passwordLabel: string; changePassword: string; nextSteps: string; step1: string; step2: string; step3: string; step4: string; btnText: string }> = {
          ...
          es: { subject: '¡Bienvenido a TaekUp! Tu acceso VIP está listo 🥋', title: '¡Tu Dojo está en línea! 🥋', greeting: 'Hola', congrats: '¡Felicitaciones! Tu acceso VIP a TaekUp ha sido aprobado. Tu club', trialNote: 'ahora está activo con una <strong>prueba gratuita de 14 días</strong>.', credTitle: '🔐 Tus credenciales de acceso:', emailLabel: 'Correo:', passwordLabel: 'Contraseña:', changePassword: '⚠️ ¡Por favor cambia tu contraseña después del primer inicio de sesión por seguridad!', nextSteps: 'Esto es lo que debes hacer a continuación:', step1: 'Inicia sesión y completa el asistente de configuración', step2: 'Agrega tus estudiantes y entrenadores', step3: 'Personaliza tu sistema de cinturones', step4: 'Configura tu billetera Stripe a través de DojoMint™ Protocol', btnText: 'Iniciar sesión en TaekUp' },
          fa: { subject: 'به TaekUp خوش آمدید! دسترسی VIP شما آماده است 🥋', title: 'دوجو شما راه‌اندازی شد! 🥋', greeting: 'سلام', congrats: 'تبریک! دسترسی VIP شما به TaekUp تأیید شد. باشگاه شما', trialNote: 'اکنون با یک <strong>آزمایش رایگان ۱۴ روزه</strong> فعال است.', credTitle: '🔐 اطلاعات ورود شما:', emailLabel: 'ایمیل:', passwordLabel: 'رمز عبور:', changePassword: '⚠️ لطفاً پس از اولین ورود، رمز عبور خود را تغییر دهید!', nextSteps: 'مراحل بعدی:', step1: 'وارد شوید و جادوگر راه‌اندازی را تکمیل کنید', step2: 'دانش‌آموزان و مربیان خود را اضافه کنید', step3: 'سیستم کمربند خود را سفارشی کنید', step4: 'کیف پول Stripe خود را از طریق DojoMint™ Protocol راه‌اندازی کنید', btnText: 'ورود به TaekUp' },
        };
        const wl = welcomeI18n[request.language || 'en'] || welcomeI18n.en;
        await sgClient.client.send({
          to: request.email,
          from: { email: 'noreply@mytaek.com', name: 'MyTaek' },
