...
- **Super Admin Dashboard**: Platform-wide management, including KPIs, club/parent management, payment history, revenue analytics, activity feed, health scores, email tools, CSV exports, and impersonation. New advanced analytics dashboard with cohort analysis, onboarding funnel, churn analysis, payment recovery, MRR goals, and automations.
    - **VIP Access Request Management**: Gated onboarding model — no public signup. Users submit a Request Access form, Super Admin reviews in Clubs → Access Requests tab, approves (auto-creates club account with 14-day trial, generates temp password, sends welcome email) or rejects. Table: `access_requests`. Endpoints: `GET/POST /api/super-admin/access-requests`, `POST /api/super-admin/access-requests/:id/approve|reject`.
